...
/** Remove hidden/reasoning blocks that some models return in their message content. */
...
export async function generateText({ system, prompt, temperature = 0.2 }) {
  ...
  if (config.provider === 'ollama') {
    ...
    if (!response.ok) throw new Error(`Ollama request failed with HTTP ${response.status}. Make sure Ollama is running and the model is available.`);
