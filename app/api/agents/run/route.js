...
export async function POST(request) {
  ...
  try {
    ...
    const { data: run, error: runError } = await supabase
      .from('agent_runs')
      .insert({ agent_id: agent.id, owner_id: user.id, status: 'running', input: { input }, started_at: new Date().toISOString() })
      .select('id')
      .single();
    if (runError) throw runError;
    runId = run.id;

    const result = await runResearchAgent({ question: input.trim(), sources: [] });
    ...
    if (runId) {
      await supabase.from('agent_runs').update({ status: 'failed', error_message: error instanceof Error ? error.message : 'Agent run failed.', completed_at: new Date().toISOString() }).eq('id', runId).eq('owner_id', user.id);
    }
