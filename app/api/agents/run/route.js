...
export async function POST(request) {
  ...
  try {
    ...
    const { error: completeError } = await supabase.from('agent_runs').update({ status: 'completed', output: { result, sourceCount: sources.length, enabledTools }, completed_at: new Date().toISOString() }).eq('id', runId).eq('owner_id', user.id);
    if (completeError) throw completeError;

    return NextResponse.json({ ok: true, result, runId, sourceCount: sources.length, enabledTools, agent: { id: agent.id, name: agent.name } });
  } catch (error) {
